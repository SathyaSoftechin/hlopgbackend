// roomController.js
import { HostelRoom } from "../models/index.js";



export const saveHostelRooms = async (req, res) => {
  ...
  try {
    const existing = await HostelRoom.findOne({ where: { hostel_id } });

    if (existing) {
      await existing.update({ layout: { floors } });
    } else {
...
export const getHostelRooms = async (req, res) => {
  try {
    ...
    const data = await HostelRoom.findOne({
      where: { hostel_id }
    });

    if (!data || !data.layout || !Array.isArray(data.layout.floors)) {
    ...
    console.error("❌ GET HOSTEL ROOMS ERROR:", err.message);
